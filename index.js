...
async function run() {
  try {
    ...
    // 5.2 send the hotjobs data to ui after that u will find the data manually by typing in browser url "http://localhost:3000/jobs"
    app.get("/jobs", async (req, res) => {
      const cursor = await jobsCollections.find().toArray();
      res.send(cursor);
    });

    // 6.3 make the api for get the data by id
